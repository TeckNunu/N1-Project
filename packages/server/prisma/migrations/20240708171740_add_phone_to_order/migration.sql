...
CREATE TABLE `User` (
    ...
    `role` ENUM('USER', 'ADMIN', 'SELLER', 'MARKETER', 'SELLERMANAGER') NOT NULL DEFAULT 'USER',
...
CREATE TABLE `Order` (
    ...
    `status` ENUM('PAYMENT_PENDING', 'PENDING', 'CONFIRMED', 'DELIVERING', 'DELIVERED', 'CANCELED') NOT NULL DEFAULT 'PENDING',
    `totalAmount` INTEGER NOT NULL,
    `notes` TEXT NULL,
    `paymentMethod` ENUM('BANK_TRANSFER', 'CASH_ON_DELIVERY') NOT NULL DEFAULT 'CASH_ON_DELIVERY',
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updatedAt` DATETIME(3) NOT NULL,
